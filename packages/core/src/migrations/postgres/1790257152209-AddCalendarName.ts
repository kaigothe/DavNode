 ...
 * backfill.
 */
export class AddCalendarName1790257152209 implements MigrationInterface {
  name = 'AddCalendarName1790257152209';

  /** Applies the migration: adds `calendars.name` and its unique index. */
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `ALTER TABLE "calendars" ADD "name" character varying NOT NULL`,
    );
    await queryRunner.query(
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `DROP INDEX "public"."IDX_26c19a8c24733f34aad43fe3cd"`,
    );
