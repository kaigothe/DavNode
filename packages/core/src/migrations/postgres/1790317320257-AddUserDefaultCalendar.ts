 ...
 */
export class AddUserDefaultCalendar1790317320257 implements MigrationInterface {
  name = 'AddUserDefaultCalendar1790317320257';

  /** Applies the migration: adds `users.default_calendar_id` and its foreign key. */
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `ALTER TABLE "users" ADD "default_calendar_id" uuid`,
    );
    await queryRunner.query(
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `ALTER TABLE "users" DROP CONSTRAINT "FK_206a9c98c895e44a2e524b03edd"`,
    );
    await queryRunner.query(
