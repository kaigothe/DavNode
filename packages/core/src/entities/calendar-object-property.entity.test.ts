import type { DataSource } from 'typeorm';
import { afterEach, beforeEach, describe, expect, it } from 'vitest';
import { createDataSource } from '../db/data-source.js';
import { ALL_MIGRATIONS } from '../migrations/sqlite/index.js';
import {
  ALL_ENTITIES,
  CalendarCollection,
  CalendarObject,
  CalendarObjectProperty,
  Principal,
  Tenant,
} from './index.js';

describe('CalendarObjectProperty entity', () => {
  let dataSource: DataSource;
  let calendarObject: CalendarObject;

  beforeEach(async () => {
    dataSource = createDataSource(
      {},
      { entities: ALL_ENTITIES, migrations: ALL_MIGRATIONS },
    );
    await dataSource.initialize();
    await dataSource.runMigrations();

    const tenant = await dataSource
      .getRepository(Tenant)
      .save(
        dataSource
          .getRepository(Tenant)
          .create({ slug: 'acme', name: 'Acme Inc.' }),
      );
    const ownerPrincipal = await dataSource
      .getRepository(Principal)
      .save(
        dataSource
          .getRepository(Principal)
          .create({ tenantId: tenant.id, kind: 'user', specialKind: null }),
      );
    const calendar = await dataSource.getRepository(CalendarCollection).save(
      dataSource.getRepository(CalendarCollection).create({
        tenantId: tenant.id,
        ownerPrincipalId: ownerPrincipal.id,
        displayName: 'Personal',
      }),
    );
    calendarObject = await dataSource.getRepository(CalendarObject).save(
      dataSource.getRepository(CalendarObject).create({
        tenantId: tenant.id,
        calendarId: calendar.id,
        name: 'event.ics',
        uid: 'uid-1',
        etag: 'etag-1',
        componentType: 'VEVENT',
        ownerPrincipalId: ownerPrincipal.id,
        dtstart: new Date('2026-09-24T10:00:00Z'),
      }),
    );
  });

  afterEach(async () => {
    await dataSource.destroy();
  });

  it('persists a dead property', async () => {
    const repository = dataSource.getRepository(CalendarObjectProperty);
    await repository.save(
      repository.create({
        calendarObjectId: calendarObject.id,
        namespace: 'DAV:',
        name: 'custom-prop',
        value: 'value1',
      }),
    );

    const found = await repository.findOneByOrFail({
      calendarObjectId: calendarObject.id,
      namespace: 'DAV:',
      name: 'custom-prop',
    });
    expect(found.value).toBe('value1');
  });

  it('rejects a duplicate (calendarObjectId, namespace, name) via plain insert', async () => {
    const repository = dataSource.getRepository(CalendarObjectProperty);
    await repository.save(
      repository.create({
        calendarObjectId: calendarObject.id,
        namespace: 'DAV:',
        name: 'custom-prop',
        value: 'value1',
      }),
    );

    await expect(
      repository.save(
        repository.create({
          calendarObjectId: calendarObject.id,
          namespace: 'DAV:',
          name: 'custom-prop',
          value: 'value2',
        }),
      ),
    ).rejects.toThrow();
  });

  it('overwrites the value instead of duplicating when set again (upsert)', async () => {
    const repository = dataSource.getRepository(CalendarObjectProperty);
    await repository.upsert(
      {
        calendarObjectId: calendarObject.id,
        namespace: 'DAV:',
        name: 'custom-prop',
        value: 'value1',
      },
      ['calendarObjectId', 'namespace', 'name'],
    );

    await repository.upsert(
      {
        calendarObjectId: calendarObject.id,
        namespace: 'DAV:',
        name: 'custom-prop',
        value: 'value2',
      },
      ['calendarObjectId', 'namespace', 'name'],
    );

    const rows = await repository.findBy({
      calendarObjectId: calendarObject.id,
      namespace: 'DAV:',
      name: 'custom-prop',
    });
    expect(rows).toHaveLength(1);
    expect(rows[0]?.value).toBe('value2');
  });

  it('allows the same property name in a different namespace', async () => {
    const repository = dataSource.getRepository(CalendarObjectProperty);
    await repository.save(
      repository.create({
        calendarObjectId: calendarObject.id,
        namespace: 'DAV:',
        name: 'color',
        value: 'red',
      }),
    );

    await expect(
      repository.save(
        repository.create({
          calendarObjectId: calendarObject.id,
          namespace: 'urn:example',
          name: 'color',
          value: 'blue',
        }),
      ),
    ).resolves.toBeDefined();
  });

  it('rejects a property on an unknown calendar object (FK constraint)', async () => {
    const repository = dataSource.getRepository(CalendarObjectProperty);

    await expect(
      repository.save(
        repository.create({
          calendarObjectId: '00000000-0000-0000-0000-000000000000',
          namespace: 'DAV:',
          name: 'x',
          value: 'y',
        }),
      ),
    ).rejects.toThrow();
  });
});
