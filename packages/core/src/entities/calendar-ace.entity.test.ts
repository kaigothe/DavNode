...
describe('CalendarAce entity', () => {
  ...
  it('persists an ACE with its privilege and grant/deny value', async () => {
    const repository = dataSource.getRepository(CalendarAce);
  ...
  it('allows several ACEs on the same calendar with distinct positions, queryable in order', async () => {
    const repository = dataSource.getRepository(CalendarAce);
